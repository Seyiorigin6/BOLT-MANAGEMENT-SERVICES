...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchBox) {
      searchBox.addEventListener('input', (e) => {
        ...
        faqItems.forEach(item => {
          ...
          if (visible && item.classList.contains('open')) {
            const ans = item.querySelector('.faq-a');
            const ans2 = item.querySelector('.faq-a');
            if (ans) setAnswerMaxHeight(ans);
          }
        });
      });
  ...
  // --- LOGIN & REGISTER LOGIC REMOVED ---
  // These are now handled by dedicated CGI scripts (cgi-login.js and register.js)
  // to prevent conflicts with the backend integration.
